export const quotesData = [
  {
    quote: {
      ko: 'Sticks elevates brands to global success, while preserving key regional nuances.',
      en: 'Sticks elevates brands to global success, while preserving key regional nuances.',
    },
    author: 'Innocean',
    role: {
      ko: 'Kim Jeong Hwan, NEXT Group',
      en: 'Kim Jeong Hwan, NEXT Group',
    },
  },
  {
    quote: {
      ko: '해외의 감도와 한국적 맥락을 모두 이해하는, 보기 드문 감각의 팀입니다.',
      en: '해외의 감도와 한국적 맥락을 모두 이해하는, 보기 드문 감각의 팀입니다.',
    },
    author: 'Sam Seoul',
    role: {
      ko: '이해승 이사님',
      en: '이해승 이사님',
    },
  },
  {
    quote: {
      ko: 'Detailed, precise word choices. I knew we met the right partner.',
      en: 'Detailed, precise word choices. I knew we met the right partner.',
    },
    author: 'SK Planet',
    role: {
      ko: 'Yun Tae Gu, Director',
      en: 'Yun Tae Gu, Director',
    },
  },
  {
    quote: {
      ko: '브랜드를 이해하는 실력과 매력적으로 표현하는 센스를 겸비한 팀',
      en: '브랜드를 이해하는 실력과 매력적으로 표현하는 센스를 겸비한 팀',
    },
    author: 'TBWA',
    role: {
      ko: '오혜주 수석국장',
      en: '오혜주 수석국장',
    },
  },
  {
    quote: {
      ko: 'Working with Sticks & Stones always adds excitement to every project!',
      en: 'Working with Sticks & Stones always adds excitement to every project!',
    },
    author: 'Dexter Krema',
    role: {
      ko: 'Song Eun Hye, Senior Director',
      en: 'Song Eun Hye, Senior Director',
    },
  },
  {
    quote: {
      ko: '각 나라마다의 독특한 문화코드를 심어 클라이언트의 사업을 성공으로 이끕니다.',
      en: '각 나라마다의 독특한 문화코드를 심어 클라이언트의 사업을 성공으로 이끕니다.',
    },
    author: 'Innocean',
    role: {
      ko: '김정환 NEXT 그룹장',
      en: '김정환 NEXT 그룹장',
    },
  },
  {
    quote: {
      ko: 'A rare team with creative sensibility fluent in global style, yet grounded in Korean insight.',
      en: 'A rare team with creative sensibility fluent in global style, yet grounded in Korean insight.',
    },
    author: 'Sam Seoul',
    role: {
      ko: 'Lee Hae-seung',
      en: 'Lee Hae-seung',
    },
  },
  {
    quote: {
      ko: '단어의 작은 뉘앙스 차이조차 놓치지 않는 섬세함. 일하는 내내 좋은 파트너를 만났다는 즐거움을 느끼게 해주었습니다.',
      en: '단어의 작은 뉘앙스 차이조차 놓치지 않는 섬세함. 일하는 내내 좋은 파트너를 만났다는 즐거움을 느끼게 해주었습니다.',
    },
    author: 'SK Planet',
    role: {
      ko: '윤태구 국장',
      en: '윤태구 국장',
    },
  },
  {
    quote: {
      ko: '클라이언트의 고민을 다각도로 점검해주셔서 내부 설득에 큰 도움이 되었어요.',
      en: '클라이언트의 고민을 다각도로 점검해주셔서 내부 설득에 큰 도움이 되었어요.',
    },
    author: 'TBWA',
    role: {
      ko: '홍민지 부장',
      en: '홍민지 부장',
    },
  },
  {
    quote: {
      ko: '스틱스앤스톤스와 함께하는 작업은 늘 즐겁다!',
      en: '스틱스앤스톤스와 함께하는 작업은 늘 즐겁다!',
    },
    author: 'Dexter Krema',
    role: {
      ko: '송은혜 차장',
      en: '송은혜 차장',
    },
  },
  {
    quote: {
      ko: '세심한 제안과 열정적 지원으로, 프로젝트 내내 든든한 파트너십을 느낄 수 있었습니다.',
      en: '세심한 제안과 열정적 지원으로, 프로젝트 내내 든든한 파트너십을 느낄 수 있었습니다.',
    },
    author: 'Innocean',
    role: {
      ko: '이유진 매니저',
      en: '이유진 매니저',
    },
  },
  {
    quote: {
      ko: 'A team that pairs brand insight with an instinct for magnetic expression.',
      en: 'A team that pairs brand insight with an instinct for magnetic expression.',
    },
    author: 'TBWA',
    role: {
      ko: 'Oh Hye-joo',
      en: 'Oh Hye-joo',
    },
  },
  {
    quote: {
      ko: '프로젝트 기간 내내 문제를 함께 고민하고 해결하려는 열정과 파트너십에 감명 받았습니다.',
      en: '프로젝트 기간 내내 문제를 함께 고민하고 해결하려는 열정과 파트너십에 감명 받았습니다.',
    },
    author: 'SK Planet',
    role: {
      ko: '박지연 플래너',
      en: '박지연 플래너',
    },
  },
  {
    quote: {
      ko: 'Detail까지 챙겨주시는 세심함과 Customized Solution을 제시해 주시는 완벽함.',
      en: 'Detail까지 챙겨주시는 세심함과 Customized Solution을 제시해 주시는 완벽함.',
    },
    author: 'SK Chemical',
    role: {
      ko: '강양리 대리',
      en: '강양리 대리',
    },
  },
  {
    quote: {
      ko: 'Sticks & Stones\' multi-angle analysis of client challenges was a huge help in getting internal approval.',
      en: 'Sticks & Stones\' multi-angle analysis of client challenges was a huge help in getting internal approval.',
    },
    author: 'TBWA',
    role: {
      ko: 'Hong Min-ji',
      en: 'Hong Min-ji',
    },
  },
];
